package de.fof1092.almostflatlandsreloaded.worldgenerator.v1_16_R2;
				...
				if (block.getType() == Material.DIRT || block.getType() == Material.GRASS_BLOCK) {
					if (blockAbove.getType() == Material.AIR) {
						...
						if (randomBlockGrass <= Options.worldGrassChance) {
							Util.setBlockFast(world, realX, blockAbove.getY(), realZ, Blocks.GRASS);
				        } else if (randomBlockFlower <= Options.worldFlowerChance) {
				        	
				        	int randomFlowerColor = random.nextInt(4) + 1;
							
				        	if (randomFlowerColor <= 3) {
				        		Util.setBlockFast(world, realX, blockAbove.getY(), realZ, Blocks.DANDELION);
				        	} else {
				        		Util.setBlockFast(world, realX, blockAbove.getY(), realZ, Blocks.POPPY);
				        	}
				        }
					}
				}
